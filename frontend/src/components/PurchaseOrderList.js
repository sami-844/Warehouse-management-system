...
function PurchaseOrderList({ onViewOrder }) {
  ...
  const load = async () => { setLoading(true); try { const params = {}; if (filterStatus) params.status = filterStatus; setOrders(await purchaseAPI.listOrders(params)); } catch(e) { console.error(e); } finally { setLoading(false); } };
  const loadSuppliers = async () => { try { setSuppliers(await supplierAPI.list({ active_only: true })); } catch(e) {} };
