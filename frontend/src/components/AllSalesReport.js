...
function AllSalesReport() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const d = await reportsAPI.allSales({ from_date: fromDate, to_date: toDate });
      setData(d);
    ...
    <div className="admin-container">
      <div className="page-header">
        <div className="header-content">
          <div className="header-icon"><ShoppingCart size={20} /></div>
          <div><h1>All Sales Report</h1><p>Complete sales invoices with KPI summary</p></div>
        </div>
      </div>
      ...
      {loading ? <LoadingSpinner text="Loading sales report..." /> : !data ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
            <div style={{ padding: '12px 20px', background: '#eff6ff', borderRadius: 8, border: '1px solid #bfdbfe', flex: 1, minWidth: 150 }}>
              <div style={{ fontSize: 11, color: '#1e40af', fontWeight: 600 }}>Total Sales</div>
              <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'var(--ds-font-mono)', color: '#1e3a5f' }}>{fmt(data.total_sales)} OMR</div>
            ...
            <div style={{ padding: '12px 20px', background: '#f0fdf4', borderRadius: 8, border: '1px solid #bbf7d0', flex: 1, minWidth: 150 }}>
              <div style={{ fontSize: 11, color: '#15803d', fontWeight: 600 }}>Total Collected</div>
              <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'var(--ds-font-mono)', color: '#166534' }}>{fmt(data.total_collected)} OMR</div>
            </div>
            <div style={{ padding: '12px 20px', background: '#fff7ed', borderRadius: 8, border: '1px solid #fed7aa', flex: 1, minWidth: 150 }}>
              <div style={{ fontSize: 11, color: '#c2410c', fontWeight: 600 }}>Returns</div>
              <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'var(--ds-font-mono)', color: '#9a3412' }}>{fmt(data.total_returns)} OMR</div>
            </div>
            <div style={{ padding: '12px 20px', background: '#fef2f2', borderRadius: 8, border: '1px solid #fecaca', flex: 1, minWidth: 150 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 700, fontFamily: 'var(--ds-font-mono)', color: '#991b1b' }}>{fmt(data.total_due)} OMR</div>
            </div>
          </div>
          ...
          <div className="table-container">
            <table className="data-table">
              ...
              <tbody>
                {data.data?.length === 0 ? (
                  <tr><td colSpan="8" className="no-data">No invoices found for this period</td></tr>
                ) : data.data?.map((r, i) => (
                  <tr key={i}>
                    <td>{i + 1}</td>
                ...
                {data.data?.length > 0 && (
                  <tr style={{ fontWeight: 700, background: '#f8fafc', borderTop: '2px solid #e2e8f0' }}>
                    ...
                    <td style={{ textAlign: 'right', fontFamily: 'var(--ds-font-mono)', color: '#16a34a' }}>{fmt(data.total_collected)}</td>
