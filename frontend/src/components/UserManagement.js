            ...
            {users.map(u => (
              <tr key={u.id} className={!u.is_active ? 'inactive' : ''}>
                ...
                <td className="action-cell">
                  <button className="edit-btn" onClick={() => editUser(u)} title="Edit">✏️</button>
                  <button className="edit-btn" onClick={() => { setShowPwdModal(u); setNewPassword(''); }} title="Password">🔒</button>
                  <button className="edit-btn" onClick={() => toggleUser(u.id)} title={u.is_active ? 'Deactivate' : 'Activate'}>{u.is_active ? '🚫' : '✅'}</button>
                </td>
              </tr>
            ))}
          </tbody>
