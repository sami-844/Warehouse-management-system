    ...
    <div className="sales-container">
      ...
      {overdue && overdue.count > 0 && (
        <div className="overdue-alert">
          <div className="overdue-header">⚠️ {overdue.count} overdue invoices — {overdue.total_overdue.toFixed(3)} OMR</div>
          <div className="overdue-list">
            {overdue.invoices.slice(0, 5).map(inv => (
              <div key={inv.id} className="overdue-item">
                ...
                <span className="oi-balance negative">{inv.balance.toFixed(3)} OMR</span>
      ...
      {payingInvoice && (
        <div className="modal-overlay" onClick={() => setPayingInvoice(null)}>
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <p>Customer: <strong>{payingInvoice.customer_name}</strong> — Balance: <strong>{payingInvoice.balance.toFixed(3)} OMR</strong></p>
            ...
            {invoices.length === 0 ? <tr><td colSpan="11" className="no-data">No invoices</td></tr> :
              invoices.map(inv => (
                <tr key={inv.id} className={inv.days_overdue > 0 ? 'overdue-row' : ''}>
                  ...
                  <td>{inv.status !== 'paid' && <button className="pay-btn" onClick={() => { setPayingInvoice(inv); setPayForm(p => ({...p, amount: inv.balance.toFixed(3)})); }}>💰</button>}</td>
                </tr>
              ))
            }
          </tbody>
